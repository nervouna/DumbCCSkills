...
状态文件保存为 `docs/brainstorming/<product-slug>-brainstorm-state.json`。`<product-slug>` 从产品名派生（小写、英文、连字符分隔）。状态文件格式：
...
收敛结果写回状态文件的 `cross_connections` 和 `open_questions` 字段。
...
| `{{PROBLEM_STATEMENT}}` | 从 warm-up 对话中提取 |
| `{{VALUE_PROPOSITION}}` | 从 warm-up 对话中提取 |
...
| `{{SCENARIO_ROWS}}` | state.scenarios，每行 `<li class="scenario-item"><span>名</span><span>用户行为</span><span>系统行为</span><span class="tag tag-p0">P0</span></li>` |
...
| `{{MVP_SCOPE}}` | Phase 2 收敛时确定的 MVP 功能点 |
| `{{VALIDATION_EXPERIMENTS}}` | Phase 2 收敛时建议的验证实验 |
| `{{RESEARCH_QUESTIONS}}` | open_questions 中 category=unknown 的条目 |

带有 `{{#HAS_XXX}}...{{/HAS_XXX}}` 的条件块：有数据时保留 `{{#HAS_XXX}}` 和 `{{/HAS_XXX}}` 之间的内容（去掉标记行）；无数据时整块替换为 empty-state。
